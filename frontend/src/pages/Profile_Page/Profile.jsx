...
//   const [newExp, setNewExp] = useState({ company_name: '', role: '', work_description: '', start_date: '', end_date: '', is_current: false });
...
//   //   try {
//   //     const response = await api.put('/website/profile/me', profile);
//   //     setProfile(response.data); // Updates score and data
...
//     const toastId = toast.loading('Saving changes...');
//     try {
//       // 1. Explicitly pick ONLY the fields the backend expects
//       const payload = {
...
//       setProfile(response.data); // Updates score and data from DB
//       toast.success('Profile updated successfully!', { id: toastId });
//     } catch (err) {
//       // 3. Improved error logging to tell you exactly what went wrong if it fails again
...
//     try {
//       // FIX: Ensure this hits the /me/photo endpoint specifically
...
//     try {
//       // FIX: Ensure this points to /me/resume
...
//   // FIX: Added Database calls to save Education
...
//       setProfile(response.data); // Update profile with new DB data
...
//     // Good - Indigo/Blue (Your original brand colors)
...
//   // FIX: Added Database calls to save Experience and fixed Date formatting
//   const handleSaveExperience = async () => {
//     if(!newExp.company_name || !newExp.role || !newExp.start_date) {
//       return toast.error("Company, Role, and Start Date are required.");
//     }

//     // Convert "YYYY-MM" to "YYYY-MM-DD" so FastAPI backend accepts it
//     const payload = {
//       ...newExp,
...
//       setProfile(response.data); // Update profile with new DB data
//       // setNewExp({ company_name: '', role: '', start_date: '', end_date: '', is_current: false });
//       // Inside handleSaveExperience, update the reset line:
...
//                   <img 
//                     // FIX: Prepend the backend URL if it's a relative path
//                     src={profile.profile_photo_url.startsWith('http') 
//                       ? profile.profile_photo_url 
//                       : `http://127.0.0.1:8000${profile?.profile_photo_url}`} 
//                     alt="Profile" 
//                     // PRO TIP: use object-cover instead of object-contain so the image fills the perfect circle without weird white gaps
...
//             </div>
//             <input 
//               type="file" ref={photoInputRef} onChange={handlePhotoUpload} accept="image/*" 
//               className="hidden" 
//             />
//           </div>
...
//                 <span className="text-xs font-bold tracking-widest uppercase text-slate-500">Profile Strength</span>
//                 <span className="text-sm font-black text-indigo-600">{profile?.profile_completion_score}%</span>
//               </div>
...
//                 {/* Dynamically apply the text color */}
//                 <span className={`text-sm font-black ${scoreTextClass}`}>
//                   {score}%
//                 </span>
//               </div>
//               <div className="w-full bg-slate-200 rounded-full h-2.5 overflow-hidden">
//                 {/* Dynamically apply the gradient bar color */}
//                 <div 
//                   className={`h-2.5 rounded-full transition-all duration-1000 ease-out ${scoreBarClass}`} 
//                   style={{ width: `${score}%` }}
//                 />
//               </div>
//             </div>
...
//                     <input 
//                       type="file" ref={fileInputRef} onChange={handleResumeUpload} accept=".pdf,.doc,.docx"
...
//                     No experience added yet. Include internships, part-time, or full-time roles.
//                   </div>
//                 ) : (
...
//                           <p className="text-slate-600 font-semibold mt-0.5 flex items-center gap-1.5"><Building size={16} className="text-slate-400"/> {exp.work_description}</p>
...
//         <div className="space-y-4">
//           <InputField 
//             label="College / University Name" id="college_name" required
//             placeholder="e.g. Delhi University" 
//             value={newEdu.college_name} onChange={(e) => setNewEdu({...newEdu, college_name: e.target.value})} 
//           />
//           <InputField 
//             label="Degree" id="degree" required
//             placeholder="e.g. BBA, MBA, B.Com" 
//             value={newEdu.degree} onChange={(e) => setNewEdu({...newEdu, degree: e.target.value})} 
//           />
//           <InputField 
//             label="Specialization / Major" id="specialization" 
//             placeholder="e.g. Business Administration / Marketing" 
//             value={newEdu.specialization} onChange={(e) => setNewEdu({...newEdu, specialization: e.target.value})} 
//           />
//           <InputField 
//             label="Graduation Year" id="graduation_year" 
//             placeholder="e.g. 2026" type="number"
//             value={newEdu.graduation_year} onChange={(e) => setNewEdu({...newEdu, graduation_year: e.target.value})} 
//           />
//           <button 
//             onClick={handleSaveEducation}
//             className="w-full mt-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl transition-colors"
//           >
//             Save Education
//           </button>
//         </div>
...
//         <div className="space-y-4">
//           <InputField 
//             label="Company Name" id="company_name" required
//             placeholder="e.g. TechCorp Sales Solutions" 
//             value={newExp.company_name} onChange={(e) => setNewExp({...newExp, company_name: e.target.value})} 
//           />
//           <InputField 
//             label="Role / Title" id="role" required
//             placeholder="e.g. Sales Intern / BDR" 
//             value={newExp.role} onChange={(e) => setNewExp({...newExp, role: e.target.value})} 
//           />
//            <div className="mb-5 w-full">
//             <label htmlFor="work_description" className="block text-xs font-bold text-slate-700 tracking-wide mb-2 uppercase">
//               Work Description
//             </label>
//             <textarea
//               id="work_description"
//               rows="4"
//               placeholder="What did you do? What were your key achievements?"
//               value={newExp.work_description}
//               onChange={(e) => setNewExp({...newExp, work_description: e.target.value})}
//               className="w-full px-4 py-3 rounded-xl placeholder-slate-400 text-slate-900 focus:outline-none transition-all duration-300 border border-slate-200 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 bg-slate-50 focus:bg-white resize-none"
//             ></textarea>
//           </div>
//           <div className="grid grid-cols-2 gap-4">
//             <InputField 
//               label="Start Date" id="start_date" type="month" required
//               value={newExp.start_date} onChange={(e) => setNewExp({...newExp, start_date: e.target.value})} 
//             />
//             <InputField 
//               label="End Date" id="end_date" type="month" disabled={newExp.is_current}
...
//             className="w-full mt-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3.5 rounded-xl transition-colors"
//           >
//             Save Experience
//           </button>
//         </div>
...
export default function ProfilePage() {
  ...
  // NEW: Added 6 new fields to initial state
  const [profile, setProfile] = useState({
    full_name: '', email: '', headline: '', city: '', country: '',
    phone_number: '', linkedin_url: '', job_role_interested: '',
    job_type: '', preferred_location: '', open_to_relocation: false,
