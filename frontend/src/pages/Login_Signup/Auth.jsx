...
//   Sparkles, GraduationCap, Mail, KeyRound
// } from 'lucide-react';
...
//       }
//     } catch (err) {
//       // If the backend returns 403 FORBIDDEN for unverified email:
//       if (err.response?.status === 403) {
//         toast.error("Email not verified. Please complete OTP verification.");
...
// // --- NEW: OTP VERIFICATION FORM COMPONENT ---
...
//     setLoading(true);
//     try {
//       const response = await api.post('/website/auth/verify-otp', { email, otp_code: otp });
//       if (response?.data?.access_token) {
//         localStorage.setItem('user_token', response.data.access_token);
//         login(response.data.access_token);
...
const OTPVerificationForm = ({ email, onSwitchToLogin }) => {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      // Removed token storage and navigation logic. 
      // Only show success message and switch to login view.
